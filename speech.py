...
def check_for_known_phrases(readable_result):
    if readable_result.find("hey computer") >= 0:
            if readable_result.find("stopp") >= 0:
                ...
                pos = readable_result.find("rollladen auf fünfzig prozent")
                res = ''.join(number_dict[ele] for ele in readable_result[pos + 1].split())
                client.publish("main/hm/manuel/rolladen", res + 15)
